...
(** [Random4.full_int bound] returns a random integer between 0 (inclusive)
     ...
     then [Random4.full_int bound] yields identical output
     across systems with varying [int] sizes.

     If [bound] is less than 2{^30},
     then [Random4.full_int bound] is equal to {!Random4.int}[ bound].

     If [bound] is at least 2{^30}
     (on 64-bit systems, or non-standard environments such as JavaScript),
     then [Random4.full_int] returns a value
     whereas {!Random4.int} raises {!Stdlib.Invalid_argument}.
...
(** [Random4.int_in_range ~min ~max] returns a random integer
...
(** [Random4.int32_in_range ~min ~max] returns a random integer
    between [min] (inclusive) and [max] (inclusive).
    Both [min] and [max] are allowed to be negative;
    [min] must be less than or equal to [max].
...
(** [Random4.nativeint_in_range ~min ~max] returns a random integer
    between [min] (inclusive) and [max] (inclusive).
    Both [min] and [max] are allowed to be negative;
    [min] must be less than or equal to [max].
...
(** [Random4.int64_in_range ~min ~max] returns a random integer
    between [min] (inclusive) and [max] (inclusive).
    Both [min] and [max] are allowed to be negative;
    [min] must be less than or equal to [max].
