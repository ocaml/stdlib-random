(* Imported from OCaml testsuite to test feature backports *)

module Test(Name:sig val name:string end)(Random:module type of Random3) = struct
...
let test name f =
  ...
  then Printf.printf "%s,%s: suspicious result\n%!" Name.name name
